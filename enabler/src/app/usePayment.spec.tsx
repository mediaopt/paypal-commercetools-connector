...
describe("PaymentProvider handleCreateOrder refreshes the payment after a failed attempt", () => {
  const initialPayment = {
    id: "payment-old",
    amountPlanned: { centAmount: 1000, currencyCode: "EUR", fractionDigits: 2 },
  } as never;
  ...
  const createPaymentCalls = () =>
    mockedProcessorRequest.mock.calls.filter(
      ([, url]) => url === "https://processor.test/payments"
    );

  it("in Checkout mode, asks for a payment again so the next attempt isn't blocked by a linked one", async () => {
    render(
      <PaymentProvider
        options={{} as any}
        requestHeader={{ "X-Session-Id": "session-1" }}
        createPaymentUrl="https://processor.test/payments"
        createOrderUrl="https://processor.test/payments/order"
        processorUrl="https://processor.test"
        shippingMethodId="standard"
    ...
    await act(async () => {
      await latestContext
        .current!.handleCreateOrder(undefined, true)
        .catch(() => {});
    });

    expect(createPaymentCalls()).toEqual([
      [{ "X-Session-Id": "session-1" }, "https://processor.test/payments", {}],
    ]);
    expect(latestContext.current!.paymentInfo.id).toBe("payment-new");
  });

  it("in legacy mode, doesn't ask for a payment again", async () => {
    render(
      <PaymentProvider
        options={{} as any}
        requestHeader={{}}
    ...
    // Legacy mode creates its payment on mount
    await waitFor(() => expect(createPaymentCalls()).toHaveLength(1));
