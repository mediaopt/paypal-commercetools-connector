 ...
 *    });
 *
 *    enabler.createComponentBuilder('PayPal')
 *      .then(builder => {
 *          const paymentElement = builder.build({
 *            showPayButton: false,
 *          });
 *          paymentElement.mount('#payment-component')
 *      })
 *
 *    enabler.createComponentBuilder('CardFields')
 *      .then(builder => {
 *          const paymentElement = builder.build({});
...
export type ComponentOptions = FormComponentProps & {
  ...
  /**
   * Whether the pay button should take the full width of its container.
   */
  fullWidth?: boolean;
  ...
  /**
   * The amount to display before a commercetools Payment exists yet (e.g. express/buy-now flows).
   */
